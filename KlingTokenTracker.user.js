...
(function () {
    ...
    function getState() {
        return deepClone({
            ...
            projectFilterEnabled: runtime.projectFilterEnabled === true,
            projectAllTimeTotal: hasActiveProject() ? getProjectAllTimeTotal(getActiveProject()) : 0,
            history,
    ...
    function resetAll() {
        ...
        runtime.projectEditorOpen = false;
        runtime.projectFilterEnabled = false;
        runtime.balance = null;
        runtime.balanceSource = 'none';
        runtime.balancePath = '';
        runtime.lastBalanceAt = null;
    ...
    function setProjectFilterEnabled(enabled) {
        runtime.projectFilterEnabled = Boolean(enabled) && hasActiveProject();
        saveUiState();
        renderSoon();
    }
    ...
    function openProjectEditor() {
        syncProjectDraftFromActive();
        runtime.projectEditorOpen = true;
        renderSoon();
    }
    ...
    function shouldCompactProject() {
        return !runtime.projectEditorOpen;
    }
    ...
    function migrateProjectLibrary() {
        ...
        syncProjectDraftFromActive();
        runtime.projectEditorOpen = !active.id && !projectLibrary.length;
    ...
    function createPanel() {
        ...
        shadow.querySelector('[data-field="projectFilterToggle"]').addEventListener('change', function (event) {
            setProjectFilterEnabled(event.currentTarget.checked);
        });
    ...
    function renderPanel() {
        ...
        const filterOn = isProjectFilterActive();
        const recentEvents = filterOn ? getFilteredHistory(activeProject) : history;
        ...
        recentEvents.slice(0, 6).forEach(function (event) {
    ...
    function renderProjectSummary(root, activeProject, hasProject, filterOn) {
        ...
        const projectTotal = getProjectAllTimeTotal(activeProject);
        setText(root, 'projectTotal', '-' + formatCredit(projectTotal));

        if (filterOn) {
            const lastProject = getProjectLastSpend(activeProject);
            if (lastLabel) lastLabel.hidden = false;
            if (lastValue) {
                lastValue.hidden = false;
                lastValue.textContent = lastProject
                    ? '-' + formatCredit(lastProject.amount) + (lastProject.estimated ? ' est.' : '')
                    : '-';
            }
        } else {
    ...
    function renderHistory(root, activeProject, hasProject, filterOn) {
        ...
        if (historyHeader) {
            ...
            if (hasProject) {
                const projectTotal = getProjectAllTimeTotal(activeProject);
                const projectCount = getProjectEventCount(activeProject);
        ...
        const displayEvents = filterOn ? getFilteredHistory(activeProject) : history;
        ...
        displayEvents.slice(0, 50).forEach(function (event) {
    ...
    function renderProjectFields(root) {
        ...
        const compact = shouldCompactProject();
        const hasProject = hasActiveProject();
        const filterOn = isProjectFilterActive();
        ...
        if (miniStat) {
            miniStat.textContent = hasProject
                ? '-' + formatCredit(getProjectAllTimeTotal(activeProject)) + ' total'
                : '';
        }
    ...
    function getActiveProject() {
        const project = sanitizeProject(runtime.project || {});
    ...
    function isProjectFilterActive() {
        return hasActiveProject() && runtime.projectFilterEnabled === true;
    }
    ...
    function getFilteredHistory(project) {
        ...
        if (!needle || !needle.name) return history.slice();
        return history.filter(function (event) {
            return eventMatchesProject(event, needle);
        });
    }

    function getProjectAllTimeTotal(project) {
        return normalizeCredit(getFilteredHistory(project).reduce(function (sum, event) {
    ...
    function backfillHistoryProjectIds() {
        if (!projectLibrary.length || !history.length) return;
        let changed = false;
